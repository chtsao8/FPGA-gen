...
int main(int argc, char **argv) {
  ...
  const int op_hcompute_tlast_write_DATA_SIZE = num_epochs*0;
  const int op_hcompute_tlast_write_BYTES_PER_PIXEL = 1 / 8;
  size_t op_hcompute_tlast_write_size_bytes = op_hcompute_tlast_write_BYTES_PER_PIXEL * op_hcompute_tlast_write_DATA_SIZE;

  total_size_bytes += op_hcompute_tlast_write_size_bytes;
  total_size_bytes_written += op_hcompute_tlast_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > op_hcompute_tlast_write(op_hcompute_tlast_write_size_bytes);
  ...
  for (int i = 0; i < op_hcompute_tlast_write_DATA_SIZE; i++) {
    ((bool*) (op_hcompute_tlast_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "unoptimized_conv_3_3_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer op_hcompute_tlast_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, op_hcompute_tlast_write_size_bytes, op_hcompute_tlast_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, op_hcompute_tlast_write_ocl_buf));
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, op_hcompute_hw_input_global_wrapper_stencil_read_ocl_buf));
...
nsduration = end - start;
  ...
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({op_hcompute_tlast_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
  ...
  std::ofstream regression_result("op_hcompute_tlast_write_accel_result.csv");
  for (int i = 0; i < op_hcompute_tlast_write_DATA_SIZE; i++) {
    regression_result << ((bool*) (op_hcompute_tlast_write.data()))[i] << std::endl;
  }
